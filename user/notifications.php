...
<tr align='center'><th align='center'><font color='red' size='6' >Notifications</font></th></tr>
<?php
//$Message='Welcome';
if($_SESSION["id"]) 
	{
		...
		//$pass=$_SESSION["pwd"];
		//$balance=0;
		$sql="select * from status where User_Id='$user'";
		$result=mysqli_query($con,$sql);
		//$i=mysql_num_rows($result);
        $row_size=mysqli_num_rows($result);
		for($i=$row_size-1;$i>=0;$i--)
		{
			$noti=mysqli_fetch_array($result);

            $Message[$i]=$noti["Notification"];
		}
		for($i=0;$i<$row_size;$i++)
		{
			echo "<tr><th align='left'><BR><font color='blue'>&nbsp;$Message[$i]</font><BR><BR></th></tr>";
		}
	}
?>
</table>
